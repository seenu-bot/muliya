...
export default function MangalsutraPage() {
  ...
  const handleAddToCart = (product: Product, e?: React.MouseEvent) => {
    e?.stopPropagation();
    addToCart({ id: product.id, name: product.name, price: product.price, originalPrice: product.originalPrice, image: product.image, collection: product.category, collectionSlug: "gold-mangalsutra-banglore" });
  ...
  const handleProductClick = (productId: string) => {
    router.push(`/products/gold-mangalsutra-banglore/${productId}`);
  };
      ...
      <div className="bg-gray-50 border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <nav className="flex items-center gap-2 text-sm text-gray-600">
            ...
            <Link href="/products/gold-mangalsutra-banglore" className="hover:text-[#E92247] transition-colors">
              Gold
            </Link>
            <ChevronRight className="w-4 h-4" />
